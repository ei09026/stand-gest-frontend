...
function resolveOutput(compilation, addedFilename, resolvedPath) {
  if (resolvedPath && resolvedPath.length) {
    compilation.assets[resolvedPath] = compilation.assets[addedFilename]; // eslint-disable-line no-param-reassign
...
function addFileToAssets(compilation, htmlPluginData, _ref) {
  ...
  return htmlPluginData.plugin.addFileToAssets(filepath, compilation).then(function (addedFilename) {
    ...
    if (hash) {
      var fileName = '';
      if (typeOfAsset.toLowerCase() === 'js') {
        fileName = addedFilename.replace('.js', '.' + suffix + '.' + typeOfAsset);
      } else if (typeOfAsset.toLowerCase() === 'css') {
        fileName = addedFilename.replace('.css', '.' + suffix + '.' + typeOfAsset);
      }

      resolvedPath = '' + resolvedPublicPath + fileName;
    } else {
      resolvedPath = '' + resolvedPublicPath + addedFilename + suffix;
    }

    htmlPluginData.assets[typeOfAsset].unshift(resolvedPath);

    resolveOutput(compilation, addedFilename, resolvedPath);
