    ...
    <div>
        <table :class="['vuetable', css.tableClass]">
                    ...
                    <template v-for="field in tableFields">
                        <template v-if="field.visible">
                            <template v-if="isSpecialField(field.name)">
                                ...
                                <th v-if="extractName(field.name) == '__component'" @click="orderBy(field, $event)" :class="['vuetable-th-component-'+trackBy, field.titleClass, {'sortable': isSortable(field)}]" v-html="renderTitle(field)"></th>
                                <th v-if="extractName(field.name) == '__slot'" @click="orderBy(field, $event)" :class="['vuetable-th-slot-'+extractArgs(field.name), field.titleClass, {'sortable': isSortable(field)}]" v-html="renderTitle(field)"></th>
                                <th v-if="extractName(field.name) == '__sequence'" :class="['vuetable-th-sequence', field.titleClass || '']" v-html="renderTitle(field)">
                                </th>
                                <th v-if="notIn(extractName(field.name), ['__sequence', '__checkbox', '__component', '__slot'])" :class="['vuetable-th-'+field.name, field.titleClass || '']" v-html="renderTitle(field)">
                                </th>
                            </template>
                            <template v-else>
                                <th @click="orderBy(field, $event)" :id="'_' + field.name" :class="['vuetable-th-'+field.name, field.titleClass,  {'sortable': isSortable(field)}]" v-html="renderTitle(field)"></th>
                            </template>
                        </template>
                    </template>
                </tr>
            ...
            <tbody v-cloak class="vuetable-body">
                <template v-for="(item, index) in tableData">
                    <tr @dblclick="onRowDoubleClicked(item, $event)" :item-index="index" @click="onRowClicked(item, $event)" :render="onRowChanged(item)" :class="onRowClass(item, index)">
                        <template v-for="field in tableFields">
                            <template v-if="field.visible">
                                <template v-if="isSpecialField(field.name)">
                                    <td v-if="extractName(field.name) == '__sequence'" :class="['vuetable-sequence', field.dataClass]" v-html="renderSequence(index)">
                                    </td>
                                    <td v-if="extractName(field.name) == '__handle'" :class="['vuetable-handle', field.dataClass]" v-html="renderIconTag(['handle-icon', css.handleIcon])"></td>
                                    ...
                                    <td v-if="extractName(field.name) === '__component'" :class="['vuetable-component', field.dataClass]">
                                        <component :is="extractArgs(field.name)" :row-data="item" :row-index="index" :row-field="field.sortField"></component>
                                    </td>
                                    <td v-if="extractName(field.name) === '__slot'" :class="['vuetable-slot', field.dataClass]" @click="onCellClicked(item, field, $event)">
                                        <slot :name="extractArgs(field.name)" :row-data="item" :row-index="index" :row-field="field.sortField"></slot>
                                    </td>
                                </template>
                                <template v-else>
                                    <td v-if="hasCallback(field)" :class="field.dataClass" @click="onCellClicked(item, field, $event)" @dblclick="onCellDoubleClicked(item, field, $event)" v-html="callCallback(field, item)">
                                    </td>
                                    <td v-else :class="field.dataClass" @click="onCellClicked(item, field, $event)" @dblclick="onCellDoubleClicked(item, field, $event)" v-html="getObjectValue(item, field.name, '')">
                                    </td>
                                </template>
                            </template>
                        </template>
                    ...
                    <template v-if="useDetailRow">
                        <tr v-if="isVisibleDetailRow(item[trackBy])" @click="onDetailRowClick(item, $event)" :class="[css.detailRowClass]">
                ...
                <template v-if="useNoResultsMessage && (!data || !data.length)">
                    <tr class="blank-row" :class="[css.detailRowClass]">
                        <td :colspan="countVisibleFields" style="text-align: center">
    ...
    export default {
        props: {
            ...
            css: {
                ...
                default () {
                    return {
                        tableClass: 'table table-striped table-hover horizontal-scroll',
        ...
        mounted () {
            ...
            if (this.loadOnStart && this.apiMode) {
                this.loadData()
            }

            if (this.isDataMode && this.data.length > 0) {
        ...
        methods: {
            ...
            loadData (success = this.loadSuccess, failed = this.loadFailed) {
                if (this.isDataMode) {
                    this.fireEvent('fetch-data', this.getAllBodyParams())
            ...
            getAllBodyParams () {
                var sortParam = this.getSortParam()
                var splitSortParam = sortParam.split('|')
                ...
                return Object.assign(params, {
                    ...
                    orderBy: {
                        column: splitSortParam[0],
                        direction: splitSortParam[1]
                    }
                })
            },
            getAllQueryParams () {
            ...
            showDetailRow (rowId) {
                if (!this.isVisibleDetailRow(rowId)) {
                    ...
                    this.$emit('update-visible-detail-rows', this.visibleDetailRows)
                }
            },
            hideDetailRow (rowId) {
                if (this.isVisibleDetailRow(rowId)) {
